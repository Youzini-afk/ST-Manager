...
function isFrontendAnchorElement(element, classify) {
  if (
    ...
    element.classList.contains("chat-message-pre-anchor")
  ) {
    return false;
  }
...
function containsFrontendAnchorElement(element, classify) {
  ...
  return Array.from(element.querySelectorAll("pre, div.TH-render")).some(
    (child) => isFrontendAnchorElement(child, classify),
  );
}
...
function chunkTopLevelNodes(container, classify) {
  ...
  Array.from(container.childNodes).forEach((node) => {
    if (
      ...
      isFrontendAnchorElement(node, classify)
    ) {
      flushNormal();
      chunks.push({
        type: "iframe",
        element: node,
        text: getFrontendTextFromElement(node),
      });
      return;
    }
    ...
    if (
      node instanceof HTMLElement &&
      containsFrontendAnchorElement(node, classify)
    ) {
      flushNormal();
...
function scanNestedFrontendAnchors(
  container,
  classify,
  options,
  anchors,
  ...
  container.querySelectorAll("pre, div.TH-render").forEach((preLike) => {
    if (
      !(preLike instanceof HTMLElement) ||
      !isFrontendAnchorElement(preLike, classify)
    ) {
      return;
    }
    ...
    anchors.push(
      mountAnchorContent(anchor, classification, text, {
        ...options,
...
function renderSegmentHtml(source, options = {}) {
  if (typeof options.renderHtml === "function") {
    return String(options.renderHtml(source, options) || "");
  }
...
function mountMarkdownSource(host, source, classify, options, anchors) {
  ...
  temp.innerHTML = renderSegmentHtml(source, options);
  ...
  chunkTopLevelNodes(temp, classify).forEach((chunk, index) => {
    ...
    if (chunk.type === "iframe") {
      ...
      anchors.push(
        mountAnchorContent(anchor, classification, chunk.text, {
          ...options,
...
export function mountMessageSegmentHost(host, options = {}) {
  ...
  const signature = JSON.stringify({
    ...
    renderHtmlSignature: String(options.renderHtmlSignature || ""),
  ...
  const classify =
    typeof options.classifyFrontendText === "function"
  ...
  if (parts.length > 0) {
    ...
    parts.forEach((part) => {
      ...
      host.appendChild(anchor);
      anchors.push(
        mountAnchorContent(anchor, part, String(part.text || ""), {
          ...options,
  ...
  if (directClassification && !source.includes("```")) {
    ...
    registry.set(host, {
      signature,
      anchors: [
        mountAnchorContent(anchor, directClassification, source, {
          ...options,
  ...
  host.innerHTML = renderSegmentHtml(source, options);
