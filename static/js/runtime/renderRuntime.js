...
function resolveHostHeight(host) {
  const rectHeight = Math.floor(host?.getBoundingClientRect?.().height || 0);
  ...
  window.addEventListener(
    ...
    () => {
      runtimes.forEach((runtime) => {
        ...
        runtime.applyMeasuredHeight(
          runtime.lastMeasuredHeight || runtime.resolveMinHeight(),
        );
      });
    },
    { passive: true },
...
class RenderIframeRuntime {
  ...
  mount() {
    ...
    iframe.addEventListener("load", () => {
      this.syncViewport();
      this.applyMeasuredHeight(
        this.lastMeasuredHeight || this.resolveMinHeight(),
      );
    });
  ...
  resolveMinHeight() {
    if (this.options.fillHostHeight) {
      return Math.max(
        24,
        resolveHostHeight(this.host) ||
          toBoundedNumber(this.options.minHeight, DEFAULT_MIN_HEIGHT),
      );
    }
    return Math.max(
      24,
  ...
  resolveMaxHeight() {
    if (this.options.fillHostHeight) {
      return Math.max(
        this.resolveMinHeight(),
        resolveHostHeight(this.host) || this.resolveMinHeight(),
      );
    }
    return Math.max(
      this.resolveMinHeight(),
      resolveMaxHeight(this.host, this.options.maxHeight),
    );
  }
  ...
  applyMeasuredHeight(height) {
    ...
    if (this.options.fillHostHeight) {
      const hostHeight = this.resolveMinHeight();
      this.lastMeasuredHeight = hostHeight;
      this.shell.style.minHeight = `${hostHeight}px`;
      this.shell.style.maxHeight = `${hostHeight}px`;
      this.shell.style.height = `${hostHeight}px`;
      this.iframe.style.minHeight = `${hostHeight}px`;
      this.iframe.style.maxHeight = `${hostHeight}px`;
    ...
    const clampedHeight = Math.max(
      this.resolveMinHeight(),
      Math.min(this.resolveMaxHeight(), numericHeight),
    );
  ...
  syncViewport() {
    ...
    const viewportHeight = this.options.fillHostHeight
      ? resolveHostHeight(this.host) || window.innerHeight
      : window.innerHeight;
